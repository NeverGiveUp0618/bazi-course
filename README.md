...
| `bazi_course_srs` | `{题号: {d:到期时间戳, i:间隔天数, s:做过次数, g:上次自评}}` |
| `bazi_course_tag_stats` | `{考点: {a:答题数, e:未答对数}}` —— 与八字日练 `bazi_cat_stats` 同构，正确率 `(a-e)/a` |
| `bazi_course_last` | `{scr, id}` 供「继续上次」 |
| `bazi_course_theme` | `null`(跟随系统) / `'light'` / `'dark'` |

自评三档 → SRS 间隔：推对 `×2.4`（首次 3 天）／部分对 `×1.1`（首次 1 天）／
没推出来 归零当天再来。**不算分**——只用来排复习间隔。
